...
class ValidateDomainEmail extends Command
{
    ...
    public function handle()
    {
        ...
        if ($emails->count() > 0) {
            ...
            $result = DomainEmail::whereIn('domain_user_contact_id', $plucked_email_array)->update(['status' => 'cron1']);
            if($result > 0){
                foreach ($emails AS $email_record) {
                    $domain_user_contact_id = $email_record->domain_user_contact_id;
                    echo $domain_user_contact_id;
                    ...
                    foreach ($emails_array AS $email) {
                        ...
                        if ($email_status == 'valid'){
                            ...
                            if($updated_as){
                                ...
                                if($user_contact_updated){
                                    DomainEmail::where('domain_user_contact_id', $domain_user_contact_id)->where('status', 'cron1')->update(['status' => 'Not Checked']);
                                }
                            }
                            break;
                        }
