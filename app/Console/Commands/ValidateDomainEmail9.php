...
class ValidateDomainEmail9 extends Command
{
     ...
     * @var string
     */
    protected $signature = 'validate:email9';
    ...
    public function handle()
    {
        ...
        if ($emails->count() > 0) {
            ...
            $result = DomainEmail::whereIn('domain_user_contact_id', $plucked_email_array)->update(['status' => 'cron9']);
                        ...
                        foreach ($emails_array AS $email) {
                            ...
                            if($email_status == ""){
                                ...
                                if(isset($response_api_array['error']) && $response_api_array['error']['code'] == 104){
                                    DomainEmail::where('status', 'cron9')->orWhere('email',$email)->update(['status' => 'unverified']);
