...
class ValidateDomainEmail7 extends Command
{
     ...
     * @var string
     */
    protected $signature = 'validate:email7';
    ...
    public function handle()
    {
        ...
        if ($emails->count() > 0) {
            ...
            $result = DomainEmail::whereIn('domain_user_contact_id', $plucked_email_array)->update(['status' => 'cron7']);
                        ...
                        foreach ($emails_array AS $email) {
                            ...
                            if($email_status == ""){
                                ...
                                if(isset($response_api_array['error']) && $response_api_array['error']['code'] == 104){
                                    DomainEmail::where('status', 'cron7')->orWhere('email',$email)->update(['status' => 'unverified']);
