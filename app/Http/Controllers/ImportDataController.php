...
//                $data =  $data->where('company_url','!=','')->where('first_name','!=','')->where('last_name','!=','')->where('full_name','!=','')->where('title','!=','')->where('company','!=','')->where('location','!=','');
                if (!empty($data) && $data->count() < 20000) {
                    ...
                    if ($sheet_id > 0) {
                        foreach ($data as $key => $value) {
                            ...
                            $full_name = (UtilString::is_empty_string($full_name)) ? NULL : $full_name;
                            $last_name = (UtilString::is_empty_string($full_name)) ? NULL : $last_name;
                            $company = (UtilString::is_empty_string($company)) ? NULL : $company;
                            $company_url = (UtilString::is_empty_string($company_url)) ? NULL : $company_url;
                            $location = (UtilString::is_empty_string($company_url)) ? NULL : $location;
                            $experience = (UtilString::is_empty_string($company_url)) ? NULL : $experience;
